...
export default function Header() {
  ...
  return (
    <>
      <header className={`${styles.header_root}${scrolled ? ` ${styles.scrolled}` : ""}`}>
        <div className={styles.header_inner}>
 
          {/* Logo */}
          <Link href="/" className={styles.logo}>
            <div className={styles.logo__icon}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="#9bc5eb" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <nav className={styles.nav_desktop} aria-label="Navegação principal">
            ...
            {/* Status ao vivo */}
            <div className={styles.status_badge} title="Dados atualizados em tempo real">
        ...
        {/* Mobile dropdown */}
        <div className={`${styles.mobile_menu}${menuOpen ? ` ${styles.open}` : ""}`} role="navigation" aria-label="Menu mobile">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
          ...
          <div className={styles.mobile_cta}>
            <Link href="/buscar" className={styles.cta_btn} style={{ justifyContent: "center" }} onClick={() => setMenuOpen(false)}>
